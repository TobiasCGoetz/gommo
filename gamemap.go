...
func (g gameMap) spreadFromSpreader(gMap *[mapWidth][mapHeight]*Tile, xCoord int, yCoord int) {
...
func (g *gameMap) spread(gMap *[mapWidth][mapHeight]*Tile) {
	for x, _ := range gMap {
		for y, tile := range gMap[x] {
			if tile.isSpreader() {
				g.spreadFromSpreader(gMap, x, y)
			}
		}
	}
}
